...
# TAG_ID = 'E280-1160-6000-0209-F811-72E3' # latest exposed one
TAG_ID = 'E280-1160-6000-0209-F811-5CE4'
PHASE_LOAD_NUM = 2
TAG_SIZE = 0.16
# antenna params
NUM_ANTENNA = 2
...
class RFID_Subscriber:
    def __init__(self):
        ...
        self.location_current = np.array([0, 0, 1]) # manually set [x, 0, y], solution from hyperbola intersection
        ...
        ### state machine ###
        self.state_status = 3 # 0 for initial
    ...
    def callback_2(self, msg):
        ...
        if msg.epc == TAG_ID:
            ...
            if self.state_status == 0:
                ### go straight for large enough rssi ###
                print('-----state 0-----')
                ...
                ### adjust the facing direction until rssi from ant1 and ant2 is approximate ###
                print('-----state 1-----')
                ...
                if msg.time - self.begin_time > 17.5 * 1000000:
                    ...
                    self.location_current = np.array([x_solution, 0, y_solution]) # use the result from self-calibration
                    # self.location_current = np.array([x_solution, 0, y_solution]) # manually set
                ...
                if self.phasor_unwrapped[msg.ant - 1] == None:
                    ...
                    if msg.ant == 2: # only send at the ant 2 callback
                        ...
                        if True:
                            ...
                            r, theta = cartesian_to_polar(tag_antref_position[0], tag_antref_position[1]) # in polar coordinate, distance (m), angle (deg)
                            ### update velocity and publish ###
                            # linear, where safety is 1 meter
                            if r > 2.1:
                                self.twist_msg.linear.x = 0.2
                            elif r > 1.1:
                                # self.twist_msg.linear.x = KP_linear * (r - 1.1) + 0.05
                                self.twist_msg.linear.x = 0.1 * (r - 1.1) + 0.1
                            elif r < 0.9:
                                self.twist_msg.linear.x = 0.0
                            # rotate
                            if abs(theta - 90) > 45:
                                self.twist_msg.angular.z = 0.54
                            else:
                                self.twist_msg.angular.z = 0.012 * (theta - 90) # oscillation exists
