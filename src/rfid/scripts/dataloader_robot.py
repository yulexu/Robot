...
class RFID_dataloader_robot:
    def __init__(self, rfid_wanted, num_of_antenna):#rfid_list_wanted = ['E280 1160 6000 0209 F811 7224'] #  ['E280 1160 6000 0209 F811 7224']['E280 6894 0000 500E 73D4 548C']
        ...
        self.data_pool_max_len = 50 # length of the pool. if too much, then consume time
    ...
    def update(self, newline):
        ...
        #print('data repetition', newline == self.data_pool[-1])
        if self.data_pool == []:
            self.data_pool.append(newline)
        else:
            #print('newline:', newline," ",self.data_pool[-1])
            #print('data repetition', (newline == self.data_pool[-1]))
            if newline[2] != self.rfid_wanted or newline == self.data_pool[-1]:
            	return np.max([self.ptime[i][0][0] for i in range(len(self.antenna_list))]), np.min([self.ptime[i][0][-1] for i in range(len(self.antenna_list))]), True
            else:
                self.data_pool.append(newline)
                    
        while len(self.data_pool) > self.data_pool_max_len:
            self.data_pool.pop(0)
        #print(len(self.data_pool))
        self.det_data = []
        self.antenna_list = []
        self.rfid_list = []
        ...
        if len(self.data_pool) == self.data_pool_max_len and self.plt_flag:
            plt.plot(self.ptime[0][0],self.phase[0][0])
            plt.plot(self.ptime[1][0],self.phase[1][0])
            #print('--------------------------------',self.ptime[1])
            plt.savefig('test%g.png' % self.cnt)
            #plt.close()
            #self.plt_flag = False
        self.cnt += 1
        self.plt_flag = self.cnt % self.data_pool_max_len == 0
    ...
    def get_data_2A(self, time_temp): ## get ONE data 
        ...
        self.phase_list = [self.phase_fit[self.a_1_idx], self.phase_fit[self.a_2_idx]]
        self.rssi_list = [self.rssi_fit[self.a_1_idx], self.rssi_fit[self.a_2_idx]]
        return self.phase_list, self.rssi_list
