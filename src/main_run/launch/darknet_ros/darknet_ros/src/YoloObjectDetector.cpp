...
YoloObjectDetector::YoloObjectDetector(ros::NodeHandle nh)
    : nodeHandle_(nh), imageTransport_(nodeHandle_), numClasses_(0), classLabels_(0), rosBoxes_(0), rosBoxCounter_(0),depthimageTransport_(nodeHandle_){
...
void YoloObjectDetector::init() {
  ...
  std::string depthcameraTopicName;
  int cameraQueueSize;
  ...
  nodeHandle_.param<std::string>("darknet_frame", darknet_frame, "");
  nodeHandle_.param("subscribers/camera_reading/topic", cameraTopicName, std::string("/camera/image_raw"));
  nodeHandle_.param("/camera/depth/image_rect_raw", depthcameraTopicName, std::string("/camera/depth/image_rect_raw"));
  ...
  depthimageSubscriber_ = depthimageTransport_.subscribe(depthcameraTopicName, cameraQueueSize, &YoloObjectDetector::depthcameraCallback, this);
  objectPublisher_ =
  ...
  pose_pub = nodeHandle_.advertise<darknet_ros_msgs::DetectionPoseList>("Detection_pose", boundingBoxesQueueSize);//move_base_simple/goal
  // Action servers.
...
void YoloObjectDetector::depthcameraCallback(const sensor_msgs::ImageConstPtr& msg) {
  ROS_DEBUG("[YoloObjectDetector] USB image received.");
  printf("***********");
  cv_bridge::CvImagePtr depth_cam_image;
  //printf("%s", msg->encoding.c_str());
  try {
    if (msg->encoding== "32FC1" ) {
      depth_cam_image = cv_bridge::toCvCopy(msg, msg->encoding);
      
    } else if ( msg->encoding == "16UC1") {
      depth_cam_image = cv_bridge::toCvCopy(msg, msg->encoding);
      
    }  else {
      ROS_ERROR("Image message encoding provided is not 32fc1, 16uc1.");
    }
  } catch (cv_bridge::Exception& e) {
    ROS_ERROR("cv_bridge exception: %s", e.what());
    return;
  ...
  if (depth_cam_image) {
    {
      ...
      depthimageHeader_ = msg->header;
      depthcamImageCopy_ = depth_cam_image->image.clone();
    }
    {
      boost::unique_lock<boost::shared_mutex> lockDepthImageStatus(mutexDepthImageStatus_);
      depthimageStatus_ = true;
    }
    depthframeWidth_ = depth_cam_image->image.size().width;
    depthframeHeight_ = depth_cam_image->image.size().height;
  }
  return;
}
...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num > 0 && num <= 100) {
    ...
    darknet_ros_msgs::DetectionPose poseMsg;
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          float pixel[2];
          pixel[0] = rosBoxes_[i][j].x* frameWidth_;
          pixel[1] = rosBoxes_[i][j].y* frameHeight_;
          d = depthcamImageCopy_.ptr<ushort>(int(pixel[1]))[int(pixel[0])];
          
          //////////////////////////////////////////////////////////////////////
          // deproject_pixel_to_point
          // publishing 2D pose message using realsense and pinhole camera model
          x = (pixel[0] - 318.66192626953125) / 618.0723876953125;
          y = (pixel[1] - 240.93887329101562) / 618.2006225585938;
          float depth_scale;
          depth_scale = 0.0010000000474974513;
                  
          
          p[0] = float(d) * x * depth_scale;
          p[1] = float(d) * y * depth_scale;
          p[2] = float(d) * depth_scale;
          //printf("%f\n",p[2]);
          //printf("%f\n",p[1]);
          ...
          poseMsg.y = -p[0];
          poseMsg.id = j;
    	    DetectionPoseListResults_.detectionposelist.push_back(poseMsg);
          //////////////////////////////////////////////////////////////////////
        }
      }
    }
    ...
    DetectionPoseListResults_.header.stamp = ros::Time::now();
    DetectionPoseListResults_.header.frame_id = "detection_all";
    DetectionPoseListResults_.image_header = headerBuff_[(buffIndex_ + 1) % 3];
    pose_pub.publish(DetectionPoseListResults_);
  } else {
    darknet_ros_msgs::ObjectCount msg;
    msg.header.stamp = ros::Time::now();
    msg.header.frame_id = "detection";
  ...
  DetectionPoseListResults_.detectionposelist.clear();
  for (int i = 0; i < numClasses_; i++) {
    rosBoxes_[i].clear();
    rosBoxCounter_[i] = 0;
  }
