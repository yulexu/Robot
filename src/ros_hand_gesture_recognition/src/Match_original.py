...
class YourClass:
        ...
        try:
            ...
            if self.last_sample_time is None or current_time - self.last_sample_time >= self.sampling_interval:
                ...
                if phasor_data_epc['phasor_unwrapped'][ant_idx] is None:
                    ...
                    self.epc_track_buffer[epc].append((timestamp, relative_distance))
                    # if epc == 'E280-1160-6000-0216-D2A7-FD55':
    ...
    def find_best_matching_epc(self):
        ...
        try:
            ...
            ant2_pos = np.array([0.125, 0])
            pos_0 = self.sort_track_buffer[0][1]
            initial_distance = np.sqrt((-pos_0[1]-ant2_pos[0])**2 +
                                       (pos_0[0] -ant2_pos[1])**2)
            ...
            for time, trajectory in zip(sort_times, sort_relative_trajectory):
                rospy.loginfo(f"Time: {time}, Relative Trajectory: {trajectory}")            
            ...
            for epc, data in self.epc_track_buffer.items():
                epc_times = [t for t, _ in data]
                relative_distances = [d for _, d in data]
                # rospy.logwarn(f"EPC: {epc} - Times and Relative Distances:")
                # for time, distance in zip(epc_times, relative_distances):
                #     rospy.loginfo(f"Time: {time}, Relative Distance: {distance}")
                
                if not epc_times or not relative_distances:
                    rospy.logerr(f"EPC: {epc} has empty time or distance data.")
                    continue

                # check if enough data for interpolate
                if len(epc_times) < 20 or len(relative_distances) < 20:
                    rospy.logwarn(f"EPC: {epc} has insufficient data points (less than 20). Skipping this EPC.")
                    continue
                # align length
                # if len(relative_distances) != len(sort_relative_trajectory):
                #     relative_distances = interpolate_data(epc_times, relative_distances, sort_times)
                
                # using DTW to match
                sort_relative_trajectory = np.array(sort_relative_trajectory)
                ...
                # rospy.logerr(f"sort:{sort_relative_trajectory}")
                # rospy.logerr(f"rfid:{relative_distances}")
                # rospy.logerr(f"sort_relative_trajectory shape: {sort_relative_trajectory.shape}, type: {type(sort_relative_trajectory)}")
                # rospy.logerr(f"relative_distances shape: {relative_distances.shape}, type: {type(relative_distances)}")

                if sort_relative_trajectory.ndim != 1 or relative_distances.ndim != 1:
                    rospy.logerr("One of the input vectors is not 1-D.")
                    continue
                # rospy.logerr(f"sort_relative_trajectory first element type: {type(sort_relative_trajectory[0])}")
                # rospy.logerr(f"relative_distances first element type: {type(relative_distances[0])}")
                distance, path = fastdtw(sort_relative_trajectory, relative_distances)
                rospy.logwarn(f"DTW distance of {epc}: {distance}")
                
                # record data
                # with open(os.path.join(data_dir, f'epc_buffer_{epc}.json'), 'w') as f:
                #     json.dump(relative_distances, f)
                # with open(os.path.join(data_dir, 'sort_relative_trajectory.json'), 'w') as f:
                #     json.dump(sort_relative_trajectory, f)

                # find matched EPC
                # distance_sum = 0
                # for i in range(len(sort_relative_trajectory)):
                #     distance_sum += abs(relative_distances[i] - sort_relative_trajectory[i])
                # rospy.logwarn(f"distance sum of {epc}: {distance_sum}")
                if distance < min_distance:
                    min_distance = distance
                    best_match_epc = epc

            if best_match_epc != None:
                rospy.logerr(f"Best Match EPC: {best_match_epc}, Distance Sum: {min_distance}")
            else:
